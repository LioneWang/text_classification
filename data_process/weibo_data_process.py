...
class WeiboEmbeddingDataset(BaseDataSet):
    """
...
class WeiboTransformersDataset(BaseDataSet):
    ...
    def __init__(self, data_dir, file_name, cache_dir, shuffle, transformer_model, overwrite_cache, batch_size,
        ...
        super(WeiboTransformersDataset, self).__init__(transformer_model=transformer_model,
    ...
    def read_examples_from_file(self):
        ...
        with input_file.open('r') as f:
            for line in tqdm(f):
                json_line = json.loads(line)
                yield InputExample(guid=json_line['id'], text=json_line['text'][:510], label=json_line['labels'])
